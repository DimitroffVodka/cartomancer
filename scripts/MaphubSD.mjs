...
// module .html as text/plain (FV14 does). Tracked blobUrl → iframe so each can
// be freed: on the iframe's `load` (its earliest safe point — once consumed, the
...
// (journal page close/re-render). Mirrors GeneratorFetcher.revokeBlobs.
	...
	try {
		...
		if (r.ok) {
			...
			if (contentType.includes("text/html")) {
				...
				// carries no ?query, so the seed would otherwise be random. Mirrors
				// MaphubViewerApp._buildSrc; genuine 404s keep the external fallback.
